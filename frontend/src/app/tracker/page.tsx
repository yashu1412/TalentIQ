...
export default function TrackerPage() {
  ...
  const [form, setForm]             = useState({ title: "", company: "", status: "saved" });
  ...
  const load = async () => {
    try {
      ...
      const [appResp, tlResp] = await Promise.all([
  ...
  const addApplication = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/applications`, { job_id: jobResp.data.job_id, status: form.status }, { headers });
      setShowModal(false);
      setForm({ title: "", company: "", status: "saved" });
      await load();
    } catch {}
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: 30, fontWeight: 800, color: "#FFFFFF" }}>Application Tracker</h1>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }}>
            <div className="glass-card p-6 w-full max-w-md animate-scale-in">
              <div className="flex items-center justify-between mb-5">
                <h2 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, color: "#FFFFFF" }}>Add Application</h2>
                <button onClick={() => setShowModal(false)} className="text-[#A1A1AA] hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="text-xs mb-1.5 block" style={{ color: "#A1A1AA" }}>Job Title</label>
                ...
                <div>
                  <label className="text-xs mb-1.5 block" style={{ color: "#A1A1AA" }}>Company</label>
                ...
                <div>
                  <label className="text-xs mb-1.5 block" style={{ color: "#A1A1AA" }}>Status</label>
                  <select value={form.status || "saved"} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}
                    className="w-full p-3 rounded-xl text-sm outline-none"
                    style={{ background: "rgba(10,10,10,0.8)", border: "1px solid #262626", color: "#FFFFFF" }}>
                    {Object.entries(STATUS_CONFIG).map(([s, c]) => (
                      <option key={s} value={s} style={{ background: "#161616" }}>{c.label}</option>
                    ))}
                  </select>
                </div>
          ...
          {Object.entries(STATUS_CONFIG).map(([status, cfg]) => {
            ...
            return (
              <div key={status} className="stat-card text-center">
                ...
                <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 22, color: "#FFFFFF" }}>{count}</div>
            ...
            return (
              <div key={status} className="glass-card p-3">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="ml-auto text-xs" style={{ color: "#52525b" }}>{cards.length}</span>
                </div>
                <div className="space-y-2">
                  {cards.map((app: any) => (
                    <div key={app.id} className="p-2.5 rounded-xl" style={{ background: "rgba(10,10,10,0.8)", border: `1px solid ${cfg.color}30` }}>
                      <p className="text-xs font-semibold" style={{ color: "#FFFFFF", marginBottom: 2 }}>
                        {app.job?.title || `Job #${app.job_id?.slice(0, 6)}`}
                      </p>
                      <p className="text-xs" style={{ color: "#52525b" }}>{app.job?.company || "—"}</p>
                      <p className="text-xs" style={{ color: "#262626", marginTop: 2 }}>{app.updated_at?.slice(0, 10)}</p>
                    </div>
                  ))}
                  {cards.length === 0 && (
                    <p className="text-xs text-center py-3" style={{ color: "#262626" }}>Empty</p>
                  )}
                </div>
              </div>
            );
        ...
        <div className="glass-card overflow-hidden">
          <div className="px-5 py-4" style={{ borderBottom: "1px solid #161616" }}>
            <h2 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, color: "#FFFFFF" }}>All Applications</h2>
          </div>
          {loading ? (
            ...
            <div className="p-10 text-center">
              <Briefcase className="w-10 h-10 mx-auto mb-3" style={{ color: "#262626" }} />
              <p style={{ color: "#52525b" }}>No applications tracked yet.</p>
              <p className="text-sm mt-1" style={{ color: "#262626" }}>Add your first application or match against a job.</p>
            </div>
          ) : (
            <div className="divide-y" style={{ borderColor: "#161616" }}>
              {applications.map((app) => {
                ...
                const Icon = cfg.icon;
                return (
                  <div key={app.id} className="flex items-center justify-between px-5 py-4 transition-colors hover:bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                      <Icon className="w-5 h-5 flex-shrink-0" style={{ color: cfg.color }} />
                      <div>
                        <p style={{ fontWeight: 600, color: "#FFFFFF" }}>{app.job?.title || `Job #${app.job_id?.slice(0, 8)}`}</p>
                        <p className="text-xs" style={{ color: "#71717a" }}>{app.job?.company || "Unknown Company"} · {app.updated_at?.slice(0, 10)}</p>
                      </div>
                    </div>
                    <select value={app.status || "saved"} onChange={e => updateStatus(app.id, e.target.value)}
                      ...
                      {Object.entries(STATUS_CONFIG).map(([s, c]) => (
                        <option key={s} value={s} style={{ background: "#161616", color: "#FFFFFF" }}>{c.label}</option>
                      ))}
                    </select>
                  </div>
                );
